OrderWeb.controller('itemCntrl', function($scope,$http,config,$rootScope,$location,$localstorage,Scopes,Facebook) {
	...
	$scope.stripe_customer_id = null;
	$scope.fk_stripe_customer_id = null;
	$scope.fk_stripe_status = 0;
	$scope.fk_stripe_percentage = 0;
	$scope.fk_stripe_amount = 0;
	$scope.fk_stripe_token = null;
	$scope.fk_order_id = null;
	$scope.fk_last4 = '';
	$scope.fk_brand = '';
				...
				$http({
					...
					data : {'restaurant_id':$localstorage.get('restaurant_id'),'location_id':$localstorage.get('location_id'),'member_id':$localstorage.get('member_id')}
						...
						if(response.status=='success'){
							...
							$scope.stripe_customer_id = response.customer_strip_id;
							$scope.fk_stripe_status = response.fk_stripe_status;
							$scope.fk_stripe_customer_id = response.fk_customer_strip_id
							console.log(response);
							if (($scope.stripe_customer_id != null && ($scope.fk_stripe_status  == 1 && $scope.fk_stripe_customer_id != null)) || 
								($scope.stripe_customer_id != null && $scope.fk_stripe_status  == 0)){ 
								console.log($scope.fk_stripe_customer_id);
								$scope.addOrder();
							} else {
								$('.checkout').show();
	...
	var stripeResponseHandler = function(status, response){
	  ...
	  $scope.addOrder = function(){
			...
			$scope.orderArray=Scopes.get('OneController');	
					...
					$scope.payment_data = ($scope.stripe_customer_id != null) ? {'cust_id': $scope.stripe_customer_id} : $scope.payment_data;
						...
						data : {'order_data':order_data}
							...
							if(response.error)
							{
								...
								if (response.amount > 0) {
									Stripe.setPublishableKey(response.forkourse_pub_key);
									$scope.fk_stripe_amount = response.amount;
									$scope.fk_order_id = response.order_id;
									$scope.forkourseCheckout();
								}
	...
	$scope.forkourseCheckout = function() {
		console.log($scope.fk_stripe_customer_id);
		if ($scope.fk_stripe_customer_id == null) {
			Stripe.card.createToken({
				  number: $scope.cardno,
				  cvc: $scope.card_cvv,
				  exp_month: $scope.card_month,
				  exp_year: $scope.card_year
			}, stripeFResponseHandler);
		} else {
			$scope.payment_data = { 'order_id' : $scope.fk_order_id, 
									'amount' : $scope.fk_stripe_amount, 
									'fk_customer_id' : $scope.fk_stripe_customer_id} ;
			var order_data = {'user_id':$localstorage.get('member_id'), 'payment_data':$scope.payment_data };
										  
						
			$http({
				method : 'POST',
				url : Urlcall+'forkourseCheckout',
				data : {'order_data':order_data}
				}).success(function(response){
					console.log(response);
	...
	var stripeFResponseHandler = function(status, response) {
		console.log(response);
		$scope.fk_stripe_token = response.id;
        $scope.fk_last4 = response.card.last4;
        $scope.fk_brand = response.card.brand;
        $scope.payment_data = { 'token' : $scope.fk_stripe_token,
								'last4' : $scope.fk_last4,
								'brand' : $scope.fk_brand, 
								'order_id' : $scope.fk_order_id, 
								'amount' : $scope.fk_stripe_amount, 
								'fk_customer_id' : $scope.fk_stripe_customer_id} ;
		var order_data = {'user_id':$localstorage.get('member_id'),  'payment_data':$scope.payment_data };
