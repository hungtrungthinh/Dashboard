OrderWeb.controller('homeCntrl', function($scope,$http,config,$rootScope,$location,Facebook,Scopes,$localstorage) {
	...
	var restaurant_id=11;
	$localstorage.set('restaurant_id',11);
	...
	$scope.selectRes = function(resname,id,city){
		...
		$http({
			...
			if(response.status=='success'){
				...
				$scope.orderType1=response.result.type;
				$scope.max_later_days=response.max_later_days;
				console.log($scope.orderType1);
			}else{	
		...
		$scope.errormsg='';
			...
			$http({
				...
				if(response.status=='success'){
						...
						$scope.currentOrderDate = orderTime;
	...
	$scope.showConfirm = function(timeAr)
	{
		...
		for (var i = 0; i < $scope.timeArray.time.length; i++)
		{
			...
			if (currentItem != timeAr)
			{
				...
				console.log($rootScope.fullDay);
				console.log('01/01/2011 ' + timeAr.start);
				console.log('01/01/2011 ' + $scope.currentOrderDate);
				
				if($rootScope.fullDay == "Today")
					{
						var selectedDate = Date.parse('01/01/2011 ' + timeAr.start);
						var currentDate = Date.parse('01/01/2011 ' + $scope.currentOrderDate);
						if(selectedDate >= currentDate)
							currentItem.showfull = true;
						else
							currentItem.showfull = false;
					}
			}
