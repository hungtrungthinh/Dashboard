...
<g>
	...
	<path fill-rule="evenodd" clip-rule="evenodd" fill="#20374A" d="M23.668,39.955c1.712,0,3.1,1.388,3.1,3.101
		c0,1.711-1.387,3.099-3.1,3.099s-3.1-1.388-3.1-3.099C20.568,41.343,21.956,39.955,23.668,39.955L23.668,39.955z"/>
								...
								<p id="password_reset_title">Enter a new password below</p>

                            </div>
                            
                            <div class="sign-in-form" id="password_reset">
    						<div class="" id="status"></div>
                                ...
                                <input type="hidden" value="<?php echo $key;?>" name="key" id="key">
                                    <label>
                                		...
                                		<input type="password" placeholder="New Password" id="txtNewPassword" name="password" required />
                                    </label>
                                    <label>
                                    	<div class="check-match">
                                        </div>
                                		<input type="password" placeholder="Re-enter new Password" id="txtConfirmPassword" name="confirm_password" onChange="checkPasswordMatch();" required />
                                    </label>
                                    <input type="submit" value="Save" id="pwd_btn" />
                                </form>
                            </div>
                            
                            <div class="sign-in-form" id="reset_success" style="display:none">
                            	<form>
                                	<p>Congratulations! Your password has been reset</p>
                                    <button class="btn-done" id="login">Sign in</button>
                                </form>
                            </div>
	...
	if(password == confirm_password){
	 ...
	 $.ajax({			
				...
				success:function (result)
				{
					...
					{
						$('#reset_success').show();
						$("#header-title").html('Password Reset');
						$('#password_reset').hide();
						$('#password_reset_title').hide();
					}
					else
					{
						$('#status').html(result);
