...
public $_table = 'fk_order_master';
	...
	public function AllOrdersCount($restaurant_id,$location_id,$role){
		...
		$sql="SELECT 
			  ...
			  (SELECT count(*) 	FROM fk_order_master  WHERE is_later='Y'  ". $query ." AND order_status ='New' AND delivery_time > '$date') as late
	...
	public function getDetailOrder($order_id){
		$sql = "SELECT MM.first_name,MM.last_name,MM.auth_type,MM.phone as 
		 member_phone,MM.email,MM.device_token,MM.device_platform,MM.allow_notification,RL.restaurant_name,RL.type,OM.order_type,DA.*,
