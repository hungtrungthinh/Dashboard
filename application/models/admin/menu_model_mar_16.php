...
public $_table = 'fk_dish_category_master';
	...
	public function UpdateOptionsSidesNew($array,$side_id='',$sideitem='',$option_id='',$num=''){
		//echo $side_id;
		if($side_id!=''){
			$sql = "SELECT count(*) as num
					FROM fk_option_sides 
					WHERE  option_id =	$option_id 
					AND side_item ='$sideitem'
					AND side_id !='$side_id'";
			$query =  $this->db->query($sql);
			$res = $query->row_array();	
			if($res['num']==0){
	...
	public function checkoptSideexist($optid,$sideid,$sideItem){
		$sql = "SELECT count(*) as num
					FROM fk_option_sides 
