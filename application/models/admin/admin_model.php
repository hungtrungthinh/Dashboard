...
public function checkStatusOfRestaurant($username,$password){
	
		$sql ="SELECT MA.admin_id,MA.full_name,MA.username,MA.`password`,MA.email,MA.role,MA.restaurant_id,MA.location_id,
			   RM.* FROM `fk_member_admins` MA 
			   LEFT JOIN  fk_restaurant_master RM on RM.restaurant_id=MA.restaurant_id 
			   where MA.username='$username' 
			   AND MA.password='$password' 
			   AND MA.role=2 
			   AND RM.status='Y' ";
...
public function checkStatusOfLocation($username,$password){
	
		$sql ="SELECT MA.admin_id,MA.full_name,MA.username,MA.`password`,MA.email,MA.role,MA.restaurant_id,MA.location_id,
			   RL.* FROM `fk_member_admins` MA 
			   LEFT JOIN  fk_restaurant_locations RL on RL.location_id=MA.location_id 
			   LEFT JOIN fk_restaurant_master RM on RM.restaurant_id=MA.restaurant_id 
			   where MA.username='$username' 
			   AND MA.password='$password' 
			   AND MA.role=3
