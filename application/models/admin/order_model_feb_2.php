...
public $_table = 'fk_order_master';
	...
	public function AllOrdersCount($restaurant_id,$location_id,$role){
		...
		$sql="SELECT 
			  ...
			  (SELECT count(*) 	FROM fk_order_master  WHERE is_later='Y'  ". $query ." AND order_status ='New'  ) as late
	...
	public function getDetailOrder($order_id){
		$sql = "SELECT MM.first_name,MM.last_name,MM.auth_type,MM.email,MM.device_token,MM.device_platform,MM.allow_notification,RL.restaurant_name,RL.type,OM.order_type,DA.*, 
				OM.order_status,OM.order_id,OM.order_ref_id,OM.created_time,OM.delivery_time,OM.tip,OM.discount_amount,OM.tax_amount,OM.sub_total,OM.refund_amount,OM.total_amount,OM.delivery_service_amount
				FROM fk_order_master OM 
