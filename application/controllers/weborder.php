...
//-------------------------umesh---------------------------------
	...
	public function login(){
			...
			if($data!=''){
				...
				$erDetails = $this->client_model->get_where('fk_member_master',array('email' => $array['email']), 'member_id ASC');
				if(empty($erDetails)){
...
//------------------------------------------------------------for website order 
	...
	public function loginWeb(){
		...
		public function gerRestaurantTimings(){
			...
			if ($result['res']['is_break']) {
				$break_startAt	=	$result['res']['break_start_at'];
				$break_endsAt	=	$result['res']['break_end_at'];

				$date = date('h:i a', strtotime($result['res']['break_start_at'].'+ 30 minutes') );
				
				$break_start	= explode(':',$result['res']['break_start_at']);
				$break_end		= explode(':',$result['res']['break_end_at']);
				// exit($OrgTime . '-' . $break_startAt . '-' . $break_endsAt);
				//========================================
				// if($OrgTime >=  $break_startAt && $OrgTime <=  $break_endsAt){
				// 	$break_curTime=$OrgTime;

				// }else if($OrgTime <  $break_startAt){
					$break_curTime=$break_startAt;
				//==================================
				// }else if($OrgTime >  $break_endsAt){
				// 	$break_curTime='next day start';
					//$endsAt= $this->nextWorkindDay($dayKey,$resp['timings']);
					//print_r($endsAt);
					//$curTime=$endsAt['start_at'];
				// }
				...
				//====================
				// if($flag==1){
				// 	$break_curTime=explode(':',$break_curTime);
				// 	$break_start[0]=$break_curTime[0];
				// 	if($break_curTime[1]>30){
				// 		$break_start[1]=00;
				// 		$break_start[0]=$break_curTime[0]+1;
				// 	}else{
				// 		$break_start[0]=$break_curTime[0];
				// 		$break_start[1]=30;
				// 	}
				...
				for($i=$break_start[0];$i<= $break_end[0];$i=$i+0.25){
					if($i==$break_start[0]){
						if($break_start[1]==30){
							$i = $i+0.50;
						}
					}
					$break_whole = floor($i).':00';      
					$break_fraction = $i - $break_whole;
					if($break_fraction == 0.25){
						$break_valstart =	date('h:i a', strtotime($break_whole.'+ 15 minutes') );
						$break_val[$j]['start'] = ltrim($break_valstart, '0');
					}else if($break_fraction == 0.50){
						$break_valstart = date('h:i a', strtotime($break_whole.'+ 30 minutes') );
						$break_val[$j]['start'] = ltrim($break_valstart, '0');
					}else if($break_fraction == 0.75){
						$break_valstart = date('h:i a', strtotime($break_whole.'+ 45 minutes') );
						$break_val[$j]['start'] = ltrim($break_valstart, '0');
					}else{
						$break_valstart= date('h:i a', strtotime($break_whole) );
						$break_val[$j]['start'] = ltrim($break_valstart, '0');
					}
					$j++;
			...
			//=========================
			// if ($flag == 1) {				
			// 	if ($startAt != '' && $startAt <= $OrgTime && $endsAt >= $OrgTime) {
			// 		foreach ($break_val as $v) {
			// 			array_push($val, $v);
			// 		}
			// 	} else {
			// 		// if ($break_startAt <= $OrgTime && $break_endsAt >= $OrgTime) {
			// 		// 	$val = $break_val;
			// 		// } else {
			// 			foreach ($break_val as $v) {
			// 				array_push($val, $v);
			...
			$result['startAt']=($startAt == '' && $break_startAt != '') ? $break_startAt : $startAt;
			$result['endsAt']=($endstAt == '' && $break_endsAt != '') ? $break_endsAt : $endsAt;
	...
	public function getResTime(){
			...
			if($startAt <=$org_time and $endsAt >=$org_time){
				...
				if($result['res']['is_break'] == 1) {
					$break_startAt = $result['res']['break_start_at'];
					$break_endsAt = $result['res']['break_end_at'];
					if ($result['res']['break_start_at'] <=$org_time and $result['res']['break_end_at'] >=$org_time) {
						$res_status="open";
						$today_time=$updated_time;
						$nextDay='Today';
					}
					else
					{
						...
						} else if($org_time  >  $endsAt && $org_time < $break_startAt){
							$t1=strtotime($break_startAt);
							$updated_time =strftime('%l:%M', $t1);	
							$today_am =  date('a',strtotime($break_startAt));
							//$updated_time =date('h:m', $updated_time);
							//$updated_time;
							//$updated_time=date('h:m',strtotime($updated_time));
							$updated_time = ltrim($updated_time, '0');
