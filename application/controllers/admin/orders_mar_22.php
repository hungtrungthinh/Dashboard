...
class Orders extends My_Controller {
	...
	public function accepted()
	{
		...
		if($this->session->userdata('user')!=''){
			...
			if($_SERVER['REQUEST_METHOD']=='POST'){
				...
				$orderdetails	=	$this->order_model->searchOrder($restaurant_id,$location_id,$order_status,$config['per_page'],$_REQUEST['per_page'],$role);
				//echo '<pre>';print_r($data['orderdetails']);exit;	
				...
				$arr=array();
				$orderdetails	=$this->order_model->getActiveOrders($restaurant_id,$location_id,$order_status,$config['per_page'],$_REQUEST['per_page'],$role);
				if(count($orderdetails)!=0){
           			foreach($orderdetails as $order){
						$this->order_model->setTable('fk_restaurant_locations');	
						$row = $this->order_model->get_by('location_id',$location_id);	
	...
	public function cancelled()
	{
		if($this->session->userdata('user')!=''){
			...
			$this->pagination->initialize($config);	
				...
				$orderdetails	=$this->order_model->getCancelledOrders($restaurant_id,$location_id,$config['per_page'],$_REQUEST['per_page'],$role);
				$arr=array();
				if(count($orderdetails)!=0){
           			foreach($orderdetails as $order){
						$this->order_model->setTable('fk_restaurant_locations');	
	...
	public function late()
	{
		if($this->session->userdata('user')!=''){
			...
			$this->pagination->initialize($config);	
				...
				$orderdetails	=$this->order_model->getLateOrders($restaurant_id,$location_id,$config['per_page'],$_REQUEST['per_page'],$role);
				$arr=array();
				if(count($orderdetails)!=0){
           			foreach($orderdetails as $order){
						$this->order_model->setTable('fk_restaurant_locations');	
	...
	public function all()
	{
		if($this->session->userdata('user')!=''){
			...
			$this->pagination->initialize($config);	
				...
				$orderdetails	=$this->order_model->getAllOrders($restaurant_id,$location_id,$config['per_page'],$_REQUEST['per_page'],$role);
				$arr=array();
				if(count($orderdetails)!=0){
           			foreach($orderdetails as $order){
						$this->order_model->setTable('fk_restaurant_locations');	
	...
	public function details_ajax(){
			...
			if(count($data['itemdetails']!='')){
					foreach($data['itemdetails'] as $var){
						...
						//echo '<pre>';print_r($sidesdetails[$var['ord_item_id']]);exit;	
						foreach($sidesdetails[$var['ord_item_id']] as $varside){
							//echo '<pre>';print_r($varside);
							$sidesdata=unserialize($varside['sides']);
	...
	public function loadNewOrder(){
		...
		if(count($data['orderdetails'])!=0){
			...
			if($this->user->role!='3'){	
			$newdata.="<td href='".$url.$order['order_id']."' style='cursor:pointer;' class='tdlink hide-mob'>".$order['restaurant_name']."</td>";
			$newtabdata.="<td href='".$url.$order['order_id']."' style='cursor:pointer;' class='tdlink hide-mob'>".$order['restaurant_name']."</td>";
			$newmobdata.="<td href='".$url.$order['order_id']."' style='cursor:pointer;' class='tdlink hide-mob'>".$order['restaurant_name']."</td>";
			}
	...
	public function send_notification($restaurant_id,$restaurant_google_api_key,$registatoin_ids, $message) {
			...
			if(!$restaurant_google_api_key)
				$restaurant_google_api_key = 'AIzaSyAyRDPitzV8OsVc_rdmjVSbeDllkx042UM';
