...
class Menu extends My_Controller {
	...
	public function category($category_id){
		...
		//print_r($_POST);exit;
			...
			$array=array(	
							...
							// 'chinese_title' => $this->input->post('chinese_title'),
			...
			foreach($data['categorylist'] as $val){
				...
				$newar['category_name'][]=$val['category_name'];
				$newar['sortorder'][]=$val['sortorder'];
			}
			$data['newar']=$newar;
			//echo '<pre>';print_r($data['newar']);exit;		
	...
	public function add_dish($cat_id,$item_id){
		...
		if($_SERVER['REQUEST_METHOD']=='POST'){
			...
			$array=array(	
							...
							'item_chinese_name'=>stripslashes(mysql_real_escape_string($this->input->post('menu_chinese_name'))),
				...
				if(count($opid)!=0){
					...
					foreach($opid as $idv){
						$dishoption=array(
											  ...
											  "option_name"=>$this->input->post('opt_title_'.$idv),
											   );
								   
											   
						$this->menu_model->setTable('fk_dish_options');
						$res	=	$this->menu_model->update_by(array('option_id'=>$idv),array("dish_item_id"=>$item_id,"option_name"=>$this->input->post('opt_title_'.$idv)));		
						//echo $this->db->last_query();echo '<pre>';print_r($dishoption);				   
						//exit;		
									   
						$optsideIDarr=  $this->input->post('opt_sideid_'.$idv);
						foreach($optsideIDarr as $sidid){
							$this->menu_model->setTable('fk_option_sides');
							$sidit=$this->input->post('opt_name_'.$sidid);
							$sidpr=$this->input->post('opt_price_'.$sidid);
							$res	=	$this->menu_model->checkoptSideexist($idv,$sidid,$sidit[0]);
							
							if($res==0){
								$res	=	$this->menu_model->update_by(array('side_id'=>$sidid),array("side_item"=>$sidit[0],"price"=>$sidpr[0]));								
							}
						}
						
						if($this->input->post('opt_name_'.$idv)!=''){
							$sidenumber	=$this->menu_model->maxSortOrderDish($idv);
							$sidenumber=$sidenumber+1;
							$sidnewit=$this->input->post('opt_name_'.$idv);
							$sidnewpr=$this->input->post('opt_price_'.$idv);
							$sidesarray=array("option_id"=>$idv,
											   "side_item"=>$sidnewit[0],
											   "price"=>$sidnewpr[0],
											   "sortorder"=>$sidenumber,
											   );
							$dishid	=$this->menu_model->insertOptionsSides($sidesarray);
			...
			if($item_id!=''){
				...
				$data['categorylist']	=	$this->menu_model->gelAllCategories($restaurant_id,$location_id);
				$data['categoryDetails']	=	$this->menu_model->get_where('fk_dish_category_master',array('category_id'=>$cat_id),"row");
				//echo '<pre>';print_r($data['categoryDetails']);
	...
	public function SaveDishItem(){
		...
		$array=array(	
						'category_id'=>$this->input->post('category'),
						'item_name'=>stripslashes(mysql_real_escape_string($this->input->post('menu_item'))),
						'item_chinese_name' => stripslashes(mysql_real_escape_string($this->input->post('item_chinese_name'))),
		...
		$newsoptionArray=$this->input->post('newsoptionArray');
		if(count($arrOptid)!=0 ){
				for($j=0;$j<count($arrOptid);$j++){
						$arraydata=array("option_name"=>$newsoptionArray[$j],
										 "mandatory"=>$arrMan[$j],
										 "multiple"=>$arrMul[$j],
										 "limit"=>$arrLimit[$j]
								   );
		...
		$newsideoptArray=$this->input->post('newsideoptArray');		
	...
	public function ajaxUpdateItem(){
		...
		if(count($options)!=0){
			for($i=0;$i<count($options);$i++){
				...
				$data['categorylist']	=	$this->menu_model->UpdateOptionsSidesNew($arr,$sidesid[$i],$options[$i],$option_id,$number);
			  // echo '<pre>';print_r($option_id);exit;
				//echo $this->load->view('admin/menu/add',$option_id);
				}
			}
