...
class Orders extends My_Controller {
	...
	public function acceptOrder(){
	   ...
	   $mailsend = $this->sendMail($order_id,'accept');
	   exit;
			...
			$data['subject']="Your order is being processed now.";
			$headers = "MIME-Version: 1.0" . "\r\n";
			$headers .= "Content-type:text/html;charset=iso-8859-1" . "\r\n";
			$headers .= 'From: <forkours@gmail.com>'. "\r\n";
			...
			//	end of posh notification 
				...
				$message	= "Your order ".$order_ref_id." is being processed now";
	...
	public function sendMail($order_id,$state){
			$this->load->model('email_model');		
			//$order_id=$_POST['order_id'];
			//$order_id=224;
			...
			//$message = str_replace('#DELADDRESS#', $result['first_name'], $message);
			//$message = str_replace('#TIP#', $result['first_name'], $message);
	...
	function sendPushNotificationIOS($message,$deviceToken,$badge_count=1,$details,$type)
		{
			...
			if($devmode=='Y'){
				stream_context_set_option($ctx, 'ssl', 'local_cert', 'pushdevcert.pem');
			}else{
				stream_context_set_option($ctx, 'ssl', 'local_cert', 'pushdistcert.pem');
			}
