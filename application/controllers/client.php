	...
	public function signUp(){
			...
			if($data!=''){
				...
				if(count($response)){
					...
					if(count($details)==0){
						$this->client_model->insert('fk_restaurant_members',array('membar_id' => $insert_id,'restaurant_id'=> $array['user_data']['restaurant_id'],'device_platform'=>$array['user_data']['device_platform'],'device_token'=>$array['user_data']['device_token'],'device_id'=>$array['user_data']['device_id']));
					}
...
//var_dump($resp);		
			if($resp['location_id'] && $resp['restaurant_id'] == $array['restaurant_id'])
			{
				...
				if(count($resp['timings'])!=0){
					...
					for($i=0;$i<7;$i++){
						foreach($resp['timings'] as $timing){
							...
							if($timing['start_at']!='' &&  $timing['end_at']!='')
							{
								...
								$timing_arrray[$day_array[$timing['day']]]['break_start_at'] = $timing['break_start_at'];
								$timing_arrray[$day_array[$timing['day']]]['break_end_at'] = $timing['break_end_at'];
								$timing_arrray[$day_array[$timing['day']]]['is_break'] = $timing['is_break'];
							}
					...
					$resp['is_break']=$timing_arrray[$day_array[$current_day]]['is_break'];
					...
					if($timing_arrray[$current_day])
					{
						...
						{	
							...
							$resp['break_start_at'] = $today_date.' '.$timing_arrray[$current_day]['break_start_at'];
							$resp['break_end_at'] = $today_date.' '.$timing_arrray[$current_day]['break_end_at'];
							$resp['is_break'] = $timing_arrray[$current_day]['is_break'];
							if($resp['end_at'] < $resp['start_at'])
							$end_time = new DateTime($tomorrow.' '.$timing_arrray[$current_day]['end_at'],new DateTimeZone($resp['timezone'])); // if time on ni8 to day
							
						}
						else if($today_end > $end_time)
						{
							if((bool)$timing_arrray[$current_day]['is_break'])
							{
								$break_start_time = new DateTime($today_date.' '.$timing_arrray[$current_day]['break_start_at'],new DateTimeZone($resp['timezone']));
								$break_today_start= new DateTime($today_date.' '.$timing_arrray[$current_day]['break_start_at'],new DateTimeZone($resp['timezone']));//for checking by subtracting delivery time 
								$break_end_time = new DateTime($today_date.' '.$timing_arrray[$current_day]['break_end_at'],new DateTimeZone($resp['timezone']));
								$break_today_end= $today_end;
								$break_today_start->modify('-'.$minutes_to_add.' minutes');
						
								if($break_end_time < $break_start_time)
									$break_end_time = new DateTime($tomorrow.' '.$timing_arrray[$current_day]['break_end_at'],new DateTimeZone($resp['timezone'])); // if time on ni8 to day
								
								if($today < $break_today_start)
								{	
									$closed = 1;
									$status = 'before_start_time';
									$resp['start_at'] = $today_date.' '.$timing_arrray[$current_day]['break_start_at'];
									$resp['end_at'] = $today_date.' '.$timing_arrray[$current_day]['break_end_at'];
									$resp['break_start_at'] = $today_date.' '.$timing_arrray[$current_day]['break_start_at'];
									$resp['break_end_at'] = $today_date.' '.$timing_arrray[$current_day]['break_end_at'];
									$resp['is_break'] = $timing_arrray[$current_day]['is_break'];
									if($resp['break_end_at'] < $resp['break_start_at'])
									$break_end_time = new DateTime($tomorrow.' '.$timing_arrray[$current_day]['break_end_at'],new DateTimeZone($resp['timezone'])); // if time on ni8 to day
									
								}
								else if($today_end > $break_end_time)
								{
									$closed = 1;
									...
									$resp['break_start_at'] = $next_date.' '.$timing_arrray[$next_day_no]['break_start_at'];
									$resp['break_end_at'] = $next_date.' '.$timing_arrray[$next_day_no]['break_end_at'];
									$resp['is_break'] = $timing_arrray[$next_day_no]['is_break'];
								}
								else
								{
									$closed = 0;							
							...
							
							{
								...
								$resp['break_end_at'] = $next_date.' '.$timing_arrray[$next_day_no]['break_end_at'];
								$resp['is_break'] = $timing_arrray[$next_day_no]['is_break'];	
								$resp['TTTTTT'] = "YYYYYY break_end_time < today_end";
							}
	...
	public function ChangeDeliveryType()
	{
		...
		if($resturant_details['status']=='success')
		{
			$result = array('status' => 'success',
							...
							'break_start_at'=>$resturant_details['result']['break_start_at'],
							'break_end_at'=>$resturant_details['result']['break_end_at'],
							'is_break'=>$resturant_details['is_break'],
				...
				if(!empty($result)){
					foreach ($result as $r)
					{
						...
						$item_array = array('item_id'=>$r['item_id'],
											'item_description'=> nl2br(stripslashes($r['item_description'])),
			...
			if($data!=''){
				...
				if(!empty($result_array)){
						...
						foreach($result_array as $val)
						{
							...
							$menu['item_description'] = nl2br(stripslashes($val['item_description']));
...
//echo date("Y-m-d H:i:s");
			...
			if($data!=''){
				...
				$forkourse_percentage = (($resdetails['forkourse_stripe_status'] == 1) && $resdetails['forkourse_stripe_percentage'] > 0 && $resdetails['forkourse_stripe_percentage'] < 100) ? $resdetails['forkourse_stripe_percentage'] : 0;
				if (($forkourse_percentage > 0) && ($forkourse_percentage * $total / 100 < 0.5)) {
					$forkourse_percentage = 0.5 / $total * 100;
				}
				$forkourse_amount = $total * $forkourse_percentage / 100;
				$owner_payment = array(
				'amount' => bcmul($total * (100 - $forkourse_percentage) / 100, 100),
				'currency' => 'usd',
				...
				$total= bcdiv($retData->amount * 100 / (100 - $forkourse_percentage), 100, 3);
				...
				$fk_stripeKey = $this->client_model->getForkourseStripeKey();
				$stripeInfo = $this->client_model->getCustomerStripInfoForForkourse($array['user_id']);
				$fk_customer_strip_id = (is_null($stripeInfo)) ? null : $stripeInfo['stripe_customer_id'];
				$result = array(
						'status' 	=> 'success',
						'order_id'	=>$order_id, 
						'forkourse_pub_key' => $fk_stripeKey['public_key'], 
						'fk_customer_id'	=> $fk_customer_strip_id, 
						'amount' 	=> $forkourse_amount, 
						'message' 	=> $this->config->item('saveOrder')
					);
		...
		public function forkourseCheckout()
		{
			$this->load->library('stripe_lib');
			...
			$fk_stripeKey = $this->client_model->getForkourseStripeKey();
			$fk_customer_id = $array['payment_data']['fk_customer_id'];
			$this->stripe_lib->setApiKey($fk_stripeKey['private_key']);
			if (!is_null($fk_stripeKey)) {
				$stripeInfo = null;
				if (!$fk_customer_id) {
					$profile_details = $this->client_model->get_where('fk_member_master',array('member_id' => $array['user_id']),'member_id');
					$customer = $this->stripe_lib->createCustomer(
						array(
							'description'=>"Customer for ".$profile_details['email'],
							'email'=>$profile_details['email'],
							'source' => $array['payment_data']['token']
					...
					$stripeInfo = array(
						...
						'last_4digit' => $array['payment_data']['last4'],
						'brand' 	  => $array['payment_data']['brand'],
						'stripe_customer_id' => $fk_customer_id,
						'create_date' => date('Y-m-d H:i:s')
					);
				}

				$retData = $this->stripe_lib->chargeCustomer(
					array(
						'amount'   => bcmul($array['payment_data']['amount'], 100),
						'currency' => 'usd',
						'customer' => $fk_customer_id
					)
				);

				$response = mysql_real_escape_string(serialize($retData));
				if (!is_null($stripeInfo)) {
					$stripeInfo['order_id'] = $array['payment_data']['order_id'];
					$stripeInfo['stripe_order_id']  = $retData->id;
					$stripeInfo['stripe_response'] = $response;
					$this->client_model->setCustomerStripInfoForForkourse($stripeInfo);
				}
				exit("Success");
			}
			echo "Error!";
...
//-------------------------umesh---------------------------------
		...
		{	
			...
			$resDetails = $this->client_model->get_where('fk_restaurant_master',array('restaurant_id' => $data['restaurant_id']),'restaurant_id');

			$fk_stripe_status = $resDetails['forkourse_stripe_status'];

			$fk_stripeInfo = $this->client_model->getCustomerStripInfoForForkourse($data['member_id']);
			$fk_customer_strip_id = (is_null($fk_stripeInfo)) ? null : $fk_stripeInfo['stripe_customer_id'];

			$result = array(
					'status' => 'success',
					'stripe_public_key' 	=>	$stripe['stripe_public_key'],
					'customer_strip_id'		=>	$customer_strip_id,
					'fk_customer_strip_id'	=>	$fk_customer_strip_id,
					'fk_stripe_status'		=>	$fk_stripe_status,
					'last_4digit'			=>	$last_4digit );
