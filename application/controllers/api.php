...
class api extends MY_Controller {
	...
	//checkFbUser
		...
		public function logout(){
			...
			$array=array("device_id"=>'',"devicePlatform"=>''); 
			$res = $this->updateDeviceTokenLogout($array,$location_id);
		...
		public function updateDeviceTokenLogout($array,$location_id)
		{
			$this->db->where(array('location_id'=>$location_id));
			$this->db->update('fk_restaurant_locations',array('device_id'=>$array['deviceId'],'device_token'=>$array['deviceToken']));
		}	
	...
	public function getTime($deliveryTime){
	   ...
	   {	
			...
			if(sizeof($data)!=0){
				...
				$refund_array = array("charge" => $strip_order_id,"amount"=>floatval($amount),"reason"=>$reason);
	...
	public function cancelOrder($arr=''){
	   ...
	   public function OrderPartialRefund($arr){
		...
		//$arr=array("order_id"=>148,"amount"=>24);
		$order_id=$arr['order_id'];
		$amount = $arr['amount'];
		...
		if($order_id && $amount>0){
			...
			if($resp['status']=='success')
			{	
				$order	=	$this->api_model->refundOrder($order_id,$resp['refund_amount']);
				$result = array('status'=>"success",'data' =>$order);
	            echo json_encode($result);
