	...
	*/
		...
		if($this->form_validation->run()){	
			...
			if(is_object($user)){
					...
					if($user->password === $password){	
						...
						$adminfolder=$this->user->root;
						if($this->input->post('isAjax')){
							echo json_encode(array('status'=>'success','message'=>'Login successfully!','adminfolder'=>$adminfolder));
							exit;
						}
						else{
							redirect('admin/home');
					...
					{
						echo json_encode(array('status'=>'error','error_message'=>'Password is invalid!'));
						exit;
					}
			}else{
				if($this->input->post('isAjax')){
					echo json_encode(array('status'=>'error','error_message'=>'Username is invalid!'));
					exit;
				}
	...
	{ 		
		if($this->is_Admin){
			...
			$output['site_name']	= 'Forkourse Admin';
		...
		$output['menus']	= getMenus($this->user->role);
		//print_r($output['menus']);
		switch($this->user->role){
			case "1" : $output['route'] = "admin";	break;
			case "2" : $output['route'] = "manager";	break;
			case "3" : $output['route'] = "chef";	break;
			default	:  $output['route'] = "admin";	break;
		}
		$id=$this->session->userdata('user')->restaurant_id;	
		$this->load->model('restaurant_model');			
		$output['userdetails']=$this->restaurant_model->getAllDetails($id);
		$output['template_url'] = $this->template_url;	
		
		
		
		
		if($this->user->role==3){			//chef
			$this->load->view('admin/chef/header',$output);
			$this->load->view('view',$output);
			$this->load->view('admin/chef/footer',$output);
		}else if($this->user->role==2){		//manager
		
			//$this->load->view('admin/admin_lte_old/header',$output);
			//$this->load->view('view',$output);
			//$this->load->view('admin/admin_lte_old/footer',$output);
			
			
			$this->load->view('admin/manager/header',$output);
			$this->load->view('view',$output);
			$this->load->view('admin/manager/footer',$output);
